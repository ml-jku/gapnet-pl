...
def scaled_elu_initialization(shape, truncated=True, dtype=tf.float32, **kwargs):
    ...
    if truncated:
        return tf.truncated_normal(shape=shape, stddev=tf.cast(tf.sqrt(1 / f_in), dtype))
    else:
        return tf.random_normal(shape=shape, stddev=tf.cast(tf.sqrt(1 / f_in), dtype))
